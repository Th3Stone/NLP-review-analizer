...
for i, review in enumerate(encoded_reviews):
    ...
    if review_len <= SEQ_LENGTH:
        ...
        # If longer than 200, truncate it (take only the first 200 words)
...
class SentimentLSTM(nn.Module):
    def __init__(self, vocab_size, output_size, embedding_dim, hidden_dim, n_layers, drop_prob=0.5):
        ...
        # take embedded vectors and process them sequentially
        # batch_first=True tells the LSTM the batch size is the first dimension of the tensor
...
vocab_size = len(word2idx) # This should be 10002 (10,000 words + PAD + UNK)
output_size = 1            # 1 node for binary classification (Positive or Negative)
embedding_dim = 400        # Size of the word vectors
hidden_dim = 256           # Number of neurons in the LSTM hidden layers
...
# Binary Cross Entropy Loss for 0/1 binary classification
...
for e in range(epochs):
    ...
    for inputs, labels in train_loader:
        ...
        # 3. Calculate the loss (squeeze the output to match the shape of the labels)
        ...
        predictions = torch.round(output.squeeze()) # Round 0.8 to 1, 0.2 to 0
    ...
    # We don't need to calculate gradients during validation (saves memory/time)
    with torch.no_grad():
